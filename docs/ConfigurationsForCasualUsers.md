...
The tools to generate these configurations consist of a single Python script that generates DAQ system configurations with different characteristics based on the configuration file given to the script. This script is `nddaqconf/scripts/nddaqconf_gen`. It uses `nddaqconf/schema/nddaqconf/confgen.jsonnet` to define the format for configuration JSON files.
...
```
nddaqconf_gen --hardware-map-file <your hardware map file> --config nddaqconf/config/nddaqconf_full_config.json daq_fake00
```
...
```
`nddaqconf_gen --config slowdown_conf.json daq_fake01`
...
```
`nddaqconf_gen --config multi_df.json daq_fake02`
